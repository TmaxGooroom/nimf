    ...
    if (g_str_has_prefix (schema_ids[i], "org.nimf.engines."))
    {
      ...
      if (g_settings_schema_has_key (schema, "active-engine"))
        active = g_settings_get_boolean (settings, "active-engine");
			g_message("load engines : %s - %s \n", engine_id, active ? "ACTIVE" : "INACTIVE");
      if (active)
      {
        NimfModule *module;
        NimfEngine *engine;
...
print_proxy (GDBusProxy *proxy)
{
  gchar *name_owner;
	...
	if (name_owner != NULL)
	{
		g_debug ("+++ Proxy object points to remote object owned by %s\n",
				name_owner);
	}
	else
	{
		g_debug ("--- Proxy object is inert - there is no name owner for the name\n");
	}
	g_free (name_owner);
}
static void
...
{
	NimfVKeyboardService* vkeyboard_proxy;
	GError *err=NULL;
  g_debug (G_STRLOC ": %s", G_STRFUNC);
  ...
  nimf_server_load_services (server);
	vkeyboard_proxy = nimf_vkeyboard_service_proxy_new_for_bus_sync(
			G_BUS_TYPE_SESSION,
			...
			NULL,
			&err);
	if(vkeyboard_proxy == NULL){
			g_message(G_STRLOC ": %s err : %s", G_STRFUNC,err->message);
	}
	g_signal_connect (vkeyboard_proxy, "notify::g-name-owner",
										G_CALLBACK (on_name_owner_notify),
										NULL);
	server->priv->vkeyboard_proxy = vkeyboard_proxy;
	g_message(G_STRLOC ": svc->priv->kbd %x , vkeyboard_proxy %x\n", G_STRFUNC, server->priv->vkeyboard_proxy, vkeyboard_proxy);
