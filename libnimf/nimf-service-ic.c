                                ...
                                const gchar   *name)
{
  ...
  g_signal_emit_by_name (server, "engine-changed", engine_id, name);
	nimf_vkeyboard_service_call_engine_change(server->priv->vkeyboard_proxy, engine_id, name, NULL,NULL,NULL);
}
...
nimf_service_ic_focus_in (NimfServiceIC *ic)
{
  ...
  server->priv->last_focused_service = nimf_service_ic_get_service_id (ic);
	nimf_vkeyboard_service_call_focus_in(server->priv->vkeyboard_proxy, server->priv->last_focused_service, NULL, NULL, NULL);
...
nimf_service_ic_focus_out (NimfServiceIC *ic)
{
  ...
  nimf_preeditable_hide (server->priv->preeditable);
	nimf_vkeyboard_service_call_focus_out(server->priv->vkeyboard_proxy, server->priv->last_focused_service, NULL, NULL, NULL);
}
